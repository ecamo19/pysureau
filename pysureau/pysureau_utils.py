...
def _create_parameter_definitions_csv_file(
    ...
    if os.path.exists(path):
        ...
        "fTRBToLeaf","Not found","Definition not found", "Not found",
        ...
        "Not found","t0","Initial date of the forcing period for leaf phenology", "Day of year",
        ...
        "latitude", "Not applicable", "Numerical value indicating the latitude of the stand", "NA",
        "longitude", "Not applicable", "Numerical value indicating the longitude of the stand", "NA",
        "year_start", "Not applicable", "Numeric indicating the start year for the simulation (must match the dates of the input climate data file)", "NA",
        "year_end", "Not applicable",   "Numeric indicating the end year for the simulation (must match the dates of the input climate data file)", "NA",
        "output_path", "Not applicable",  "Path to folder where a CSV output result file will be saved. By default `None` which means that no file is written", "NA",
        "time_step_for_evapo", "Not applicable", "Numerical value (in hours) indicating the time step for the main evapotranspiration loop. Should be one of the following 1,2,4,6,8", "NA",
        "resolution_output", "Not applicable",   "Resolution chosen to write variables in files, `subdaily`, `daily` or `yearly`.", "NA",
        "output_type", "Not applicable",         "The output variables of the model that should be written in the output model file.","NA",
        "output_overwrite", "Not applicable",    "Boolean indicating whether the output result file can be overwritten if it already exists", "NA",
        "defoliation", "Not applicable",          "Boolean indicating whether trees should loose leaves when`occurs.cavitation` occurs of the above part of plant. Defoliation starts only when PLC_Leaf > 10% .", "NA",
        "threshold_mortality", "Not applicable",  "Percentange value indicating the percentage loss of conductivity above which the plant is considered dead and simulation stops for the current year.", "NA",
        "transpiration_model", "Not applicable",  "Transpiration model type. Either  or 'granier'" , "NA",
        "transpiration_granier_arg_a", "Not applicable", "Unknown parameter definition", "NA",
        "transpiration_granier_arg_b", "Not applicable", "Unknown parameter definition", "NA",
        "transpiration_granier_arg_c", "Not applicable", "Unknown parameter definition", "NA",
        "pet_formulation", "Not applicable",  "Formulation of ETP to be used, either `pt` (Priestley-Taylor) or `penman` (Penmman)", "NA",
        "rn_formulation", "Not applicable",   "Method to be used to calculate net radiation from global radiation, either `linacre`  or 'linear' (the linear method is not implemnted yet)", "NA",
        "constant_climate", "Not applicable",  "Boolian indicating whether the climate should be considered constant or not", "NA",
        "comp_options_for_evapo", "Not applicable", "Option to be used for the loops (voir avec Francois). Choose between 'normal', 'accurate','fast' or 'custom'", "NA",
        "custom_small_time_step_in_sec", "Not applicable",  "Time step in seconds. Use if comp_options_for_evapo is set to `custom`", "NA",
        "lcav", "Not applicable", "Boolean (0/1) to offset cavitation release from the apoplasm of leaves", "NA",
        "scav", "Not applicable", "Boolean (0/1) to offset cavitation release from the apoplasm of stem","NA",
        "eord", "Not applicable", "Unknown parameter definition", "NA",
        "numerical_scheme", "Not applicable", "Method to be used, either implicit, semi-implicit (Xu) or explicit", "NA",
        "stomatal_reg_formulation", "Not applicable",  "Type of regulation to be used for stomatal response to leaf symplasmic water potential, either `sigmoid`, `piecewise_linear` or 'turgor'", "NA",
        "print_prog","Not applicable", "Print progress of simulation to console (T/F)?", "NA",
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            # Write header
            writer.writerow(['parameter_name_in_paper', 'parameter_name_in_Rcode', 'definition', 'units'])

            for _ in range(4, list_lenght + 1, 4):
        
                # Write data rows
                row_value = definitions_list[current_position : (current_position + 4)]
